"""Where JEditor's log goes, and that importing the package writes nothing (workspace item X-6)."""
...
from je_editor.utils.logging import loggin_instance
from je_editor.utils.logging.loggin_instance import (
    LOG_FILE_ENV,
    JEditorLoggingHandler,
    default_log_file,
)

MODULE_FILE = Path(loggin_instance.__file__)

# 在全新的直譯器裡單獨載入這個模組（不經過會拉進 Qt 的套件 __init__），再列出工作目錄。
...
def test_default_is_under_the_home_directory(monkeypatch, tmp_path):
    ...
    assert default_log_file() == tmp_path / ".je_editor" / "logs" / "JEditor.log"
...
def test_first_record_creates_the_directory_and_appends(tmp_path):
    target = tmp_path / "nested" / "JEditor.log"
    _log(JEditorLoggingHandler(filename=str(target), delay=True), "first")
    _log(JEditorLoggingHandler(filename=str(target), delay=True), "second")
...
def test_text_outside_cp950_and_lone_surrogates_are_kept(tmp_path):
    target = tmp_path / "JEditor.log"
    _log(JEditorLoggingHandler(filename=str(target)), "繁體 ⠐ \U0001F600 \udcff")
...
def test_an_unopenable_path_turns_file_logging_off(tmp_path):
    ...
    handler = JEditorLoggingHandler(filename=str(blocker / "JEditor.log"), delay=True)
